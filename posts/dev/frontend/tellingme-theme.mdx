...
type: Dev
date: 2023-05-02
tags: ['tellingme', 'react', 'typescript']
---
...
텔링미의 디자인 스타일가이드대로 `color`,`shadow`,`gradient`,`font` 들을 모아놓은 Theme을 생성한다.
...
`DefaultTheme`에 있는 `common` 파일은 `flex`를 사용할때 자주 사용하는 스타일끼리 뭉쳐있다.
...
한번 정의해놓은 `Grid`로 불필요한 `div`를 생성할 필요없이 `Grid`를 `import`하여 사용하면 된다.
...
디자인 스타일 가이드를 따라 `Text`도 같은 폴더에 만들었다.
...
`color`와 `typo`는 앞서 말한 `theme`와 매칭 시켜주는 역할을 한다.
...
1. 리팩토링 시 `style.Grid`가 너무 많아 한눈에 알아보기 힘들다.
2. VScode의 autoTag가 <strong>마침표</strong>에 focus되어 수정이 필요없는 컴포넌트도 건들게 된다.

### 해결 방법

1. `style.Grid`를 상속받는 `styled-components`를 만든다.
2. `Grid`와 `Text`를 구조분해하여 받아 사용한다.
...
프론트엔드 개발자들`(개발자라기보단 같이 성장하는 동료들이지만)`은 어떤 한 주제나 코드에 대해 말할 때, 어떻게 하는 것이 <strong>맞아요?</strong> 가 아니라 <strong>좋아요?</strong> 나 <strong>나아요?</strong>라고 물어본다.
그리고 답할때도 마찬가지로 경험에 기반하여 장단점들을 토론하여 나에게 맞는 식의 개발방식을 택하여 가는 것 같다.
...
남은 개발 기간동안 팀원들과 열심히 개발하여 좋은 성과를 냇으면 좋겠다. 화이팅!
