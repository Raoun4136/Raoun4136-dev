...
```ts
export interface LogEventLogInterfaceParams {
  entry_point: string;
  ...
}

const EventLogInterface = (
  params: LogEventLogInterfaceParams,
  analytics: Analytics // firebase analytics
) => logEvent(analytics, 'EVENT_LOG_1', params);
```

위와 같이 `LogEventLogInterfaceParams` 라는 인터페이스를 정의하고, `EventLogInterface` 함수를 통해 로깅 이벤트를 정의하고 있다.
...
const App = () => {
  ...
  return <button>Click Event!</button>;
};
```
